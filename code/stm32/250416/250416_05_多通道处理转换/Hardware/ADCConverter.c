...
//	ADC_RegularChannelConfig(ADC1,ADC_Channel_1,2,ADC_SampleTime_41Cycles5);
	...
	ADCInitStruct.ADC_ExternalTrigConv = ADC_ExternalTrigConv_None;//外部触发转换，ADC使能后需要一个事件触发，ADC才开始转换，NONE不适用外部转换，内部写代码实现
...
//获得采集到的值
uint16_t getConvertVal(uint8_t ADC_channel)
{
	//配置规则组通道，传参进行配置，即配置ADCConverter_Init初始化的第4步，后一个通道和序列覆盖前一个
