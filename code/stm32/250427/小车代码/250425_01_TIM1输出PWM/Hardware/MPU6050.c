...
void MPU6050_Write(uint8_t RegAdd,uint8_t data)
{
	...
	while(I2C_CheckEvent(I2C2,I2C_EVENT_MASTER_TRANSMITTER_MODE_SELECTED) != SUCCESS);//EV6事件
...
void getMpu6050Data(MPU6050_Data *MPU6050_speedRegData)
{
	...
	MPU6050_speedRegData->GyroX =  MPU6050_Read(GYRO_XOUT_H) << 8 | MPU6050_Read(GYRO_XOUT_L);
	MPU6050_speedRegData->GyroY =  MPU6050_Read(GYRO_YOUT_H) << 8 | MPU6050_Read(GYRO_YOUT_L);
	MPU6050_speedRegData->GyroZ =  MPU6050_Read(GYRO_ZOUT_H) << 8 | MPU6050_Read(GYRO_ZOUT_L);
}
