...
void PWM1_Init(void)
{
	...
	TIM_OCInitStruct.TIM_Pulse = 0;	//设置占空比
...
void PWM2_Init(void)
{
	...
	TIM_OCInitStruct.TIM_OCPolarity = TIM_OCPolarity_High;	//输出波形的极性不变
	TIM_OCInitStruct.TIM_OutputState = TIM_OutputState_Enable;	//使能输出
	TIM_OCInitStruct.TIM_Pulse = 0;	//设置占空比，此处设置为0，外部使用	TIM_SetCompare1(TIM2,30);	进行实时设置占空比
	
	TIM_OC2Init(TIM2,&TIM_OCInitStruct);	//给TIM2的OC1配置输出比较
	//5.配置GPIO----使用TIM2_CH2 = PA1
	GPIO_InitTypeDef GPIO_InitStruct;
	GPIO_InitStruct.GPIO_Mode = GPIO_Mode_AF_PP;	//复用推挽输出，片上外设（定时器）帮输出高低电平，不是手动使用write或GPIO_SetBits设置
