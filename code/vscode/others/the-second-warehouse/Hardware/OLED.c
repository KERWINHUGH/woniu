...
/**
  ...
  * @retval 无
  */
void OLED_Show_ch(uint8_t Line, uint8_t Column,int num)
{
	uint8_t i;
	OLED_SetCursor((Line - 1) * 2, (Column - 1) * 16);		//设置光标位置在上半部分
	for (i = 0; i < 16; i++)
	{
		OLED_WriteData(OLED_WENZI[num][i]);			//显示上半部分内容
	}
	OLED_SetCursor((Line - 1) * 2 + 1, (Column - 1) * 16);	//设置光标位置在下半部分
	for (i = 0; i < 16; i++)
	{
		OLED_WriteData(OLED_WENZI[num][i+16]);		//显示下半部分内容
	}
}
