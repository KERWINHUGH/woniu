...
char * name_verifi( char * namebuf){
	...
	int reti;
	regex_t regex;				//正则变量
	char *user_reg = "^[a-z|A-Z|0-9]{4,12}$";	// 用户名规则：以大写字母开头，包含大小写和数字的4-12位字符
...
char * password_verifi( char * passbuf){
	...
	int reti;
	regex_t regex;				//正则变量
	char *user_reg = "^[A-Z][a-z|A-Z|0-9]{3,11}$";	// 密码规则：以大写字母开头，包含大小写和数字的4-12位字符
	...
	if(ret_val != NULL ){
		...
		if(reti == 0){
			...
			//用户名不符合规则
			puts("对不起，用户密码是一大写字母开头的，包含大小写字母和数字的4-12位字符！");
			//printf("输入的是：%s \n",namebuf);
			return NULL;	
		}else {
			//正则匹配失败
