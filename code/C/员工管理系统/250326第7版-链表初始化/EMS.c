...
void sys_init(){
	...
	*/
	...
	print_list(head);
	/*
    head = add_node(head, p[0], 1);    // 在链表的头部插入节点1
    head = add_node(head, p[1], 2);    // 在链表的头部插入节点2
    head = add_node(head, p[2], 3);    // 在链表的头部插入节点3
    head = add_node(head, p[3], 4);    // 在链表的头部插入节点4
    print_list(head);
    */
    /*
	//初始化头结点
	Node *head = createNode(p[0]);	
	printf("%d\n",Admin_quantity);	
	addNode(head,p[1]);	
	addNode(head,p[2]);	
	//addNode(head,p[3]);	
	*/
	...
	/*
	for(int i=1; i< Admin_quantity; i++){
		//将查询到的用户结构体数组元素逐个插入到链表中
		addNode(head,p[i]);	
