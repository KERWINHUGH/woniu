...
Node *add_node(Node *head, Admin data, int index){
    // Node *head，链表头指针
    // int data，要插入的数据
    // int index，要插入的位置
    if(index == 1){
...
void change_node(Node *head, int old_data, int new_data){
    // 修改链表中包含指定数据的节点的数据域
    Node *temp = search_node(head, old_data);  // 查找包含修改数据的节点
    if(temp == NULL){
        printf("链表中没有找到包含数据%d的节点\n", old_data);
        return;
    }
    temp->data.id = new_data;   // 修改节点数据
}
