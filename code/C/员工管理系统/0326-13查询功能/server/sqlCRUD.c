...
int sql_Employee_update( char *sql, Param params[],  int length ){
	...
	for(int i = 0; i < length; i++){
		if( params[i].type == Type_int ){
			...
			sqlite3_bind_text(stmt , 4, params[i].value.name , -1 , SQLITE_STATIC );
			sqlite3_bind_text(stmt , 5, params[i].value.password , -1 , SQLITE_STATIC );
			//共用体枚举赋值

			const char* gender_str = gender_to_string(params[i].value.genders);
			sqlite3_bind_text(stmt, 6, gender_str, -1, SQLITE_STATIC);
	
			const char* department_str = department_to_string(params[i].value.departments);
			sqlite3_bind_text(stmt, 7, department_str, -1, SQLITE_STATIC);
			
			const char* title_str = title_to_string(params[i].value.titles);
			sqlite3_bind_text(stmt, 8, title_str, -1, SQLITE_STATIC);
			
			const char* role_str = role_to_string(params[i].value.roles);
			sqlite3_bind_text(stmt, 9, role_str, -1, SQLITE_STATIC);
		}
	}
...
sqlite3_stmt* selectEmployeeData( char *sql, Param params[],  int length ){
	...
	if(params != NULL){	
		...
		for(int i = 0; i < length; i++){
			if( params[i].type == Type_int ){
				sqlite3_bind_int(stmt , 1, params[i].value.id );
				sqlite3_bind_int(stmt , 2, params[i].value.age );
				sqlite3_bind_int(stmt , 3, params[i].value.salary );
			}else if( params[i].type == Type_string ){
				sqlite3_bind_text(stmt , 4, params[i].value.name , -1 , SQLITE_STATIC );
				sqlite3_bind_text(stmt , 5, params[i].value.password , -1 , SQLITE_STATIC );
				//共用体枚举赋值
