...
void *writeLog( void *log){

	//现将穿进来的参数改为char*类型
	//char *user_name = (char *)username;
	char *user_log= (char *)log;
...
void *getUserInfo(void *username){
    ...
    char file_path[100];
    sprintf(file_path, "%s/%s", dir_path, user_name);  // 得到文件路径
...
int login(){
    ...
    while(flag1=='1'){
        ...
        //使用线程
        pthread_t readFile;		//创建读取用户文件信息的线程
		if(pthread_create( &readFile, NULL, getUserInfo, username) != 0){
			perror("创建线程失败！");
			return 1;
		}
		...
		User *current = NULL;
		//回收线程
		if(pthread_join(readFile,(void **)&current) != 0){
			perror("回收线程失败！");
			return 1;
		}		
        ...
        if(strcmp(password,current->password)==0){
            ...
            if(current->status == 0){
				...
				if(pthread_create( &writeLogpid, NULL, writeLog, "登录") != 0){
					perror("创建线程失败！");
					return 1;
				}
        ...
        if(error == 3){
			...
			pthread_t writeLogpid;		//创建记录日记信息的线程
			if(pthread_create( &writeLogpid, NULL, writeLog, "账户被锁定") != 0){
				perror("创建线程失败！");
				return 1;
			}
			pthread_join(writeLogpid,NULL);		//回收线程
...
void displayAllUsersInfo(){
    ...
    while((ptr = readdir(dir)) != NULL){
        ...
        //使用线程
        pthread_t readFile;		//创建读取用户文件信息的线程
		if(pthread_create( &readFile, NULL, getUserInfo, ptr->d_name) != 0){
			perror("创建线程失败！");
			return ;
		}
		//回收线程
		if(pthread_join(readFile,(void **)&userinfo) != 0){
			perror("回收线程失败！");
			return ;
		}		
    ...
    pthread_t writeLogpid;		//创建记录日记信息的线程
			if(pthread_create( &writeLogpid, NULL, writeLog,  "获取所有用户") != 0){
				perror("创建线程失败！");
				return ;
			}
	pthread_join(writeLogpid,NULL);		//回收线程
...
void displayUserInfo(){
    ...
    pthread_t writeLogpid;		//创建记录日记信息的线程
			if(pthread_create( &writeLogpid, NULL, writeLog,  "查询详细信息") != 0){
				perror("创建线程失败！");
				return ;
			}
	pthread_join(writeLogpid,NULL);		//回收线程
...
int deleteUser(){
    ...
    pthread_t writeLogpid;		//创建记录日记信息的线程
			if(pthread_create( &writeLogpid, NULL, writeLog,  "删除用户") != 0){
				perror("创建线程失败！");
				return 1;
			}
	pthread_join(writeLogpid,NULL);		//回收线程
...
int modifyUserInfo(int choice){
    ...
    if(choice == 2 || choice == 5){
        ...
        strcpy(userinfo->password, new);
         ...
         pthread_t writeLogpid;		//创建记录日记信息的线程
			if(pthread_create( &writeLogpid, NULL, writeLog,  "修改密码") != 0){
				perror("创建线程失败！");
				return 1;
			}
		pthread_join(writeLogpid,NULL);		//回收线程
        ...
        pthread_t writeLogpid;		//创建记录日记信息的线程
			if(pthread_create( &writeLogpid, NULL, writeLog,  "修改手机号") != 0){
				perror("创建线程失败！");
				return 1;
			}
		pthread_join(writeLogpid,NULL);		//回收线程
        ...
        pthread_t writeLogpid;		//创建记录日记信息的线程
			if(pthread_create( &writeLogpid, NULL, writeLog,  "修改地址") != 0){
				perror("创建线程失败！");
				return 1;
			}
		pthread_join(writeLogpid,NULL);		//回收线程
        ...
        pthread_t writeLogpid;		//创建记录日记信息的线程
			if(pthread_create( &writeLogpid, NULL, writeLog,  "修改用户状态") != 0){
				perror("创建线程失败！");
				return 1;
			}
		pthread_join(writeLogpid,NULL);		//回收线程
        ...
        pthread_t writeLogpid;		//创建记录日记信息的线程
			if(pthread_create( &writeLogpid, NULL, writeLog,  "解除锁定") != 0){
				perror("创建线程失败！");
				return 1;
			}
		pthread_join(writeLogpid,NULL);		//回收线程
