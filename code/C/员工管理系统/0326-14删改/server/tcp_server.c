...
void server_write( int client_fd,char *op_str, char *json_buf){
	...
	switch(op){
		...
		//puts("server_write----------------------3333333");
			serverDeleteEmployee(p);
		//	puts("server_write----------------------44444444");
			break;
		case UPDATE_EMPLOYEE:
			puts("serverUpdateEmployee----------------------1111111---------------------------------------------\n");
			serverUpdateEmployee(p);
			puts("serverUpdateEmployee----------------------22222222*****************************************\n");
			break;
		case INQUR_EMPLOYEE:
			//查询后返回结构体数组的大小
...
void serverDeleteEmployee(Employee  *p){
	...
	Param params[1]= {0};
	params[0].Type = Type_string;
	params[0].value.name = p[0].name;
	
	//求出操作中的param结构体包含几个成员,即表中有几个字段
	int length = sizeof(params) / sizeof(params[0]);
	//执行数据库新增成员操作
	//puts("serverDeleteEmployee----------------------5555555");
	int rows = sql_Employee_update(sql, params , length );
	//puts("serverDeleteEmployee----------------------6666666");
	if(rows < 0){
		printf( "数据库新增成员出错！\n" );
		return;
	}else{
...
void serverUpdateEmployee(Employee  *p){
	printf("----========--serverUpdateEmployee---==============--");
	// 修改
	char sql[] = "update Employee set  name = ?, password = ? ,age = ? , salary = ? , gender = ? , department = ? , title = ? , role = ? where name = ? ";	
	//定义参数类型和参数内容，然后将客户端传来的用户数据放入param结构体中（param结构体用于数据库的增删改查）
	Param params[8];
	params[0].Type = Type_string;
	params[0].value.name = p->name;
