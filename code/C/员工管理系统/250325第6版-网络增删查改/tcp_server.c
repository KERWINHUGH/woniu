...
void *server_read(void *args){
	...
	while(1){
		...
		if(strcmp(op_buf,"INQUR_ADMIN") == 0){
...
void server_write( int client_fd,char *op_str, char *json_buf){
	...
	Admin  *p =NULL;
	if(op == ADD_ADMIN || op == DELETE_ADMIN || op == MODIFY_ADMIN){
			//将传进来的json字符串转为对应结构体类型
		 p = json_to_Admin(json_buf);
	}
	//通过操作选择相应的数据库操作
	switch(op){
		case ADD_ADMIN:
			//参数传入，使用数据库操作对数据库Admin修改
			serverAddAdmin(p);
			break;
		case DELETE_ADMIN:
			clientDeleteAdmin(p);
		...
		case INQUR_ADMIN:
			//查询后返回结构体数组的大小
			p = clientInqurAdmin(&Admin_quantity);
			break;
		default:
			printf("没有该操作");
			break;	
	...
	if(op == ADD_ADMIN || op == DELETE_ADMIN || op == MODIFY_ADMIN){
		...
		Admin *p = clientInqurAdmin(&Admin_quantity);
	}
	/*
	printf("%d",Admin_quantity);
	...
	*/
	//向客户端发送结构体大小消息	
	int len = htonl(Admin_quantity);//将长度转为网络字节序
...
void clientDeleteAdmin(Admin  *p){
	...
	char *name = p->name;
	//根据用户名删除用户
	char sql[] = "delete from Admin where name = ? ";	
	//定义参数类型和参数内容，然后将客户端传来的用户数据放入param结构体中（param结构体用于数据库的增删改查）
	Param params[1];
	params[0].type = Type_string;
	params[0].value.name = name;
...
void clientUpdateAdmin(Admin  *p){
	...
	char *password = p->password; 
	// 修改
	char sql[] = "update Admin set name = ?, password = ? where name = ?";	
	//定义参数类型和参数内容，然后将客户端传来的用户数据放入param结构体中（param结构体用于数据库的增删改查）
	Param params[2];
	params[0].type = Type_string;
	params[0].value.name = name;
