...
int sql_Employee_update( char *sql, Param params[],  int length ){
	...
	//puts("serverDeleteEmployee----------------------7777777\n");
	//printf("length============:%d\n",length);
    for (int i = 0; i < length; i++) {
        ...
        //    printf("Type_stringhhhhhhhhhhhhhhh============:%d\n",Type_string);
        }else if (params[i].Type == Type_password) {
       // printf("Type_password============:%d\n",Type_password);
            // 假设Param的value.str_value存储密码
          //  printf("Type_password=======222222=====:%d\n",Type_password);
			sqlite3_bind_text(stmt , param_index, params[i].value.password , -1 , SQLITE_STATIC );
        } else if (params[i].Type == Type_gender) {
            // 枚举类型转字符串
           // printf("Type_gender============:%d\n",Type_gender);
   ...
   // puts("serverDeleteEmployee----------------------8888888");
    int retries = 3;
	while (retries--) {
	...
	//puts("serverDeleteEmployee----------------------9999999");
	/*	
	// 执行sql
