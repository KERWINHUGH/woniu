...
Employee *clientDeleteEmployee(Node * head,int client_fd){
	...
	//通过json先转换用户信息
	char* json_str = Employee_to_json( &cur->data );
	printf("发送删除信息：%s",json_str);
	//向服务器发送用户信息
	if(write(client_fd, json_str, strlen(json_str)) < 0){
		perror("向服务端发送用户信息失败！\n");
	}	
...
Employee * client_read(int client_fd ,int *employee_quantity){
	...
	*employee_quantity = ntohl(*employee_quantity);
	...
	/*
	int len ; 
	len = sizeof(Employee )*(*employee_quantity);
	int total_sent = 0;
    while (total_sent < len) {
        int count = send(client_fd, json_str + total_sent, len - total_sent, 0);
        if (count <= 0) {
           // if (errno == EINTR) continue; // 处理系统中断
            perror("client_read failed");
            return NULL;
        }
        total_sent += count;
    }
