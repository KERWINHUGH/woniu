...
void toTime(Time *timeRecord);//显示时间
...
char *getTime()
{
	...
	//格式化日期和时间，但不包括星期
	strftime(t, 128, "%Y%m%d%H%M%S", now);
...
void initUserData()
{
	...
	timeRecord=(Time *)malloc(sizeof(Time)*2);	
	//初始时间
	strcpy(timeRecord[0].year,"1900");
	strcpy(timeRecord[0].month,"1");
	strcpy(timeRecord[0].day,"1");
	strcpy(timeRecord[0].hour,"0");
	strcpy(timeRecord[0].minute,"0");
	strcpy(timeRecord[0].scecond,"0");
...
void initTime()
{
	...
	strcpy(timeRecord[1].year,s1);
	strcpy(timeRecord[1].month,s2);
	strcpy(timeRecord[1].day,s3);
	strcpy(timeRecord[1].hour,s4);
	strcpy(timeRecord[1].minute,s5);
	strcpy(timeRecord[1].scecond,s6);
	...
	for(int i=0;i<len;i++)
	{
		...
		if(i<4)
		{
			timeRecord[1].year[i]=t[i];
		}else if(i<6)
		{
			i+=4;
			timeRecord[1].month[i]=t[i];
		}else if(i<8)
		{
			i+=7;
			timeRecord[1].day[i]=t[i];
		}else if(i<10)
		{
			i+=9;
			timeRecord[1].hour[i]=t[i];
		}else if(i<12)
		{
			i+=11;
			timeRecord[1].minute[i]=t[i];
		}else if(i<14)
		{
			i+=13;
			timeRecord[1].scecond[i]=t[i];
		}		
	}
	*/
...
void toTime(Time *timeRecord)
{
	printf("上次登录时间：%s年%s月%s日 %s:%s:%s \n", timeRecord->year, timeRecord->month, timeRecord->day, timeRecord->hour, timeRecord->minute, timeRecord->scecond );
}
