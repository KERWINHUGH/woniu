...
void change_node(Node *head, Node *old_username, Node *new_username){
    // 修改链表中包含指定数据的节点的数据域
    Node *temp = searchUser(head, old_username->data->username);  // 查找包含修改数据的节点
    if(temp == NULL){
        printf("链表中没有找到包含数据%s的节点\n", old_username->data->username);
        return;
    }
    strcpy(temp->data->username , new_username->data->username);   // 修改节点数据
}
                ...
                switch(suc_choice){
                    ...
                    case 2:
		                if(temp == NULL){
		                    ...
		                    Node *new = initList();
		                    printf("请输入用户名");
		                    scanf(" %s",new->data->username);
		                    change_node(head, temp, new);
                        break;
                    case 3:
                        break;
                }
