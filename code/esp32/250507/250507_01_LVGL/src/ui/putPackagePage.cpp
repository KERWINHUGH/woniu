...
static void btnm_event_handler(lv_event_t * e)
{
    ...
    else if(strcmp(txt, LV_SYMBOL_NEW_LINE) == 0) 
    {
        ...
        //1.从textarea控件中获取文本内容，也就是用户输入的手机号
        const char * phoneNum = lv_textarea_get_text(ta);
        //
        const char * boxName = "zhudongdong";
        //2.创建一个JSON对象，生成JSON的字符串
        cJSON * root = cJSON_CreateObject();
        cJSON_AddStringToObject(root,"boxName",boxName);
        cJSON_AddStringToObject(root,"phone",phoneNum);
        char * json_str = cJSON_PrintUnformatted(root);

        Serial.printf("%s",json_str);
        //3.调用MQTT发布函数给主题发送信息
        //发送MQTT请求
        mqttPublish("woniuSms",json_str);
        
        //4.发送数据后将textarea输入框中内容清空，并释放json对象占用的内存
...
void createPutPackagePage(lv_obj_t *parent)
{
    ...
    lv_textarea_set_placeholder_text(ta,"请输入手机号");
