...
void server_write( int client_fd,char *op_str, char *json_buf){
	...
	switch(op){
		...
		case GENDER_BY_DEPARTEMNT:
			//**显示指定部门的男女员工人数
			p = genderByDepartment(&Employee_quantity);
...
void serverAddEmployee(Employee  *p){
	...
	params[2].value.name = p->age;
	params[3].type = Type_int;
	params[3].value.password =  p->salary;
	
	params[4].type = Type_string;
	params[4].value.gender = gender_to_string(p->genders);
	params[5].type = Type_string;
	params[5].value.department = department_to_string(p->departments);
	params[6].type = Type_string;
	params[6].value.title =  title_to_string(p->titles);
	params[7].type = Type_string;
	params[7].value.role =   role_to_string(p->roles);
...
Employee *  genderByDepartment(int *Employee_quantity){
	//根据用户名部门查询
	...
	Param params[1];
	params[0].type = Type_string;
	params[0].value.name = department_to_string(p->departments);
	//求出操作中的param结构体包含几个成员,即表中有几个字段
	int length = sizeof(params) / sizeof(params[0]);
	//执行数据库新增成员操作
	int rows = sql_Employee_update(sql, params , length );
	if(rows < 0){
		printf( "数据库查询部门男女人数出错！\n" );
		return;
	}else{
		printf("sql执行影响行数是：%d \n", rows );
	} 
