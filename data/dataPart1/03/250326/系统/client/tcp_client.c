...
//创建客户端连接并发送用户操作
Employee * create_client(OP op, int *Employee_quantity){
	//1.创建套接字
	//第三个参数:为0代表自动选择
	...
	Employee *Employee = client_write( client_fd, op,Employee_quantity);
	close(client_fd);	
	return Employee;
}	

//客户端通信发送用户Employee的操作与用户数据
Employee* client_write(int client_fd ,OP op, int *Employee_quantity){
	//客户端进行交互
	...
	switch(op){
		case ADD_Employee:	
				//增Employee操作
				puts("增Employee操作");
				p = clientAddEmployee(client_fd);
				return p;	 
		case DELETE_Employee:
				//删Employee操作
				puts("删Employee操作");
				p = clientDeleteEmployee(client_fd);
				return p;
		case MODIFY_Employee:
				//改Employee操作
				puts("改Employee操作");
				p = clientUpdateEmployee(client_fd);
				return p;
		case INQUR_Employee:
				//查Employee操作
				puts("查Employee操作");
				p = clientInqurEmployee(client_fd,Employee_quantity);
				 return p;;			
		default:
			puts("op error!");
			return NULL;	
...
//跟新管理员用户操作-----改操作
Employee *clientUpdateEmployee(int client_fd){
	char str[BUFF_SIZE];	//客户端发送消息的缓冲区	
	char *ret_name ;		//判断用户名验证
	char *ret_pass ;		//判断用户密码验证
...
Employee * client_read(int client_fd ,int *Employee_quantity){
	...
	*Employee_quantity = ntohl(*Employee_quantity);
	...
	if(count >0){
		...
		printf("服务器说：%s\n",json_str);
		//得到结构体数组
		Employee *p = json_to_Employee_array( json_str, &count);
		/*
		puts("得到数组");
		//循环遍历数据表结构体数组判断该用户密码是否正确
