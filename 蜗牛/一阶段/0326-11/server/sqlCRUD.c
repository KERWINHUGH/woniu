...
int sql_Employee_update( char *sql, Param params[],  int length ){
	...
	for(int i = 0; i < length; i++){
		if( params[i].type == Type_int ){
			...
			sqlite3_bind_int(stmt , i + 1, params[i].value.age );
			sqlite3_bind_int(stmt , i + 1, params[i].value.salary );
			...
			//共用体枚举赋值
			sqlite3_bind_text(stmt , i+ 1, gender_to_string(params[i].value.genders ), -1 , SQLITE_STATIC );
			sqlite3_bind_text(stmt , i+ 1, department_to_string(params[i].value.departments ), -1 , SQLITE_STATIC );
			sqlite3_bind_text(stmt , i+ 1, title_to_string(params[i].value.titles ), -1 , SQLITE_STATIC );
			sqlite3_bind_text(stmt , i+ 1, role_to_string(params[i].value.roles ), -1 , SQLITE_STATIC );
		}
	}
...
Employee* dataToEmployee( sqlite3_stmt *stmt , int *count ){
	...
	while(sqlite3_step(stmt) == SQLITE_ROW){
		...
		p[index].salary = sqlite3_column_int(stmt,4);
		//枚举操作
